...
## 7. Связь с целями задания

* **Воспроизводимость**: все шаги — от обучения до инференса — описаны кодом и запускаются с нуля; параметры и метрики фиксируются в MLflow.
* **Deepchecks**: реализована валидация качества данных и разбиения train/test, отчёты сохраняются и легко интерпретируются.
* **EvidentlyAI**: выполнен анализ дрейфа между train и test с HTML-отчётом.
* **MLflow**: эксперименты регистрируются с параметрами, метриками и артефактами; при наличии MinIO артефакты уходят в S3.
* **CI/CD**: настроены пайплайны GitHub Actions и GitLab CI для автоматической проверки кода и тестов.
* **API и контейнеризация**: FastAPI-приложение разворачивается через Docker, в продовом режиме используется Gunicorn + UvicornWorker.
* **Секреты**: все ключи и конфиденциальные настройки вынесены в `.env`.
